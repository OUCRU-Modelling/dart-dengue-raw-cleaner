...
hospital_plot_dat <- lazy_dt(s3_cleaned_incidence_dat) %>%
  group_by(date, in_out_patient, hospital) %>%
  tally() %>%
  group_by(hospital, .add = TRUE) %>%
  complete(fill = list(n = NA)) %>%
  mutate(
    year = year(date),
    isoweek = isoweek(date),
  ) %>%
  group_by(year, isoweek, in_out_patient, hospital) %>%
  summarise(date = min(date), n = sum(n)) %>%
  ungroup() %>%
  as_tibble()

hospital_lvl <- hospital_plot_dat %>%
  group_by(hospital) %>%
  summarise(hosp_count = sum(n, na.rm = TRUE)) %>%
  arrange(desc(hosp_count)) %>%
  pull(hospital)
...
hospital_plot_dat %>%
  ...
  mutate(
    ...
    hospital_label = fct(hospital, levels = hospital_lvl) %>%
fct_lump_prop(0.05)
  ) %>%
  group_by(date, in_out_patient, hospital_label) %>%
  summarise(count = sum(n, na.rm = TRUE)) %>%
  ungroup() %>%
  complete(date, in_out_patient, hospital_label, fill = list(count = NA)) %>%
  ggplot(aes(x = date, y = count, color = in_out_patient)) +
...
"",
  ...
  ggtitle("Weekly dengue incidence per hospital per case status")


ggsave("plots/hosp_dist_plot.jpg", width = 10)
```
