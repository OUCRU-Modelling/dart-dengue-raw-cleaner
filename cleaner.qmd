...
xlsx_raws[[2]] <- list(
  suppressWarnings(read_excel(
    "incidence_data_2000_2022/DART_GỬI_2017-2022_update.xlsx"
  )) %>%
    clean_names()
)
```
...
```{r}
raw_2017_2022 <- xlsx_raws[[2]] |> bind_rows()
```
...
```{r}
s3_cleaned_incidence_dat %>%
  filter(between(date, as.Date("2017-01-01"), as.Date("2023-01-01"))) %>%
  tabyl(in_out_patient)
```
...
s3_cleaned_incidence_dat |>
  arrange(date) |>
  write_csv("out/incidence_full_2000_2020.csv")
```
...
s3_cleaned_incidence_dat %>%
  ...
  group_by(week, in_out_patient, district) %>%
  tally() %>%
  ungroup() %>%
  complete(week, in_out_patient, district) %>%
